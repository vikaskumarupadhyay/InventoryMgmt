    ...
    public partial class salesordersearch : Form
    {
        ...
        private void salesordersearch_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string actualcolumn = "SELECT top 1 dbo.customerorderdescriptions.orderid, dbo.orderdetails.date,dbo.CustomerDetails.custId, dbo.CustomerDetails.CustName,dbo.CustomerDetails.CustCompName,dbo.customerorderdescriptions.ItemId ,dbo.ItemDetails.ItemName,dbo.ItemDetails.ItemCompName,itd.HSN,cast(dbo.ItemPriceDetail.MrpPrice as numeric(38,2)),cast(dbo.customerorderdescriptions.price as numeric(38,2)),dbo.customerorderdescriptions.quantity,cast(dbo.customerorderdescriptions.price *dbo.customerorderdescriptions.quantity as numeric(38,2)),itd.Discount,cast((dbo.customerorderdescriptions.price*itd.Discount)/100 as numeric(38,2)),cast((((dbo.customerorderdescriptions.Price)* (dbo.customerorderdescriptions.Quantity))-(dbo.customerorderdescriptions.Price)* (dbo.customerorderdescriptions.Quantity)*(itd.Discount/100))as numeric(38,2)),( case when DBo.CustomerDetails.CustState != DBO.CompnayDetails.State then '0.0'else CGST end) as CGST,(case when DBo.CustomerDetails.CustState != DBO.CompnayDetails.State then '0.0'else SGST end) as SGST,(case when DBo.CustomerDetails.CustState = DBO.CompnayDetails.State then '0.0' else itd.IGST  end) as IGST, itd.CESS, cast((dbo.customerorderdescriptions.totalammount)-((dbo.customerorderdescriptions.totalammount*dbo.orderdetails.Discount)/100)as numeric(38,2)),(case when exists  ( select orderid from salesOrderDelivery where Orderid=  orderdetails.orderid)then 'Delivered'else 'Pending'end) FROM dbo.CompnayDetails CROSS JOIN dbo.ItemDetails INNER JOIN dbo.customerorderdescriptions ON dbo.ItemDetails.ItemId = dbo.customerorderdescriptions.ItemId INNER JOIN dbo.ItemPriceDetail ON dbo.ItemDetails.ItemId = dbo.ItemPriceDetail.ItemId Join ItemTaxDetail itd on dbo.ItemPriceDetail.ItemId=itd.ItemId INNER JOIN dbo.orderdetails ON dbo.customerorderdescriptions.orderid = dbo.orderdetails.orderid INNER JOIN dbo.CustomerDetails ON dbo.orderdetails.custid = dbo.CustomerDetails.custId Order by orderid asc";
        ...
        private void dateTimePicker2_ValueChanged(object sender, EventArgs e)
        {
            ...
            //string selectquery = "SELECT dbo.CustomerDetails.custId as[Customer ID], dbo.CustomerDetails.CustName as[Customer Name], dbo.CustomerDetails.CustCompName as[Compnay Name],dbo.CustomerDetails.CustAddress as [Address], dbo.CustomerDetails.CustCity as[City], dbo.CustomerDetails.CustState as[State], dbo.CustomerDetails.CustZip as[Zip], dbo.CustomerDetails.CustCountry as [Country],dbo.CustomerDetails.CustEmail as[Email], dbo.CustomerDetails.CustWebAddress as[Web Address], dbo.CustomerDetails.CustPhone as[Phone], dbo.CustomerDetails.CustMobile as [Mobile],dbo.customerorderdescriptions.orderid as [Order ID], dbo.orderdetails.date as [Order Date],dbo.customerorderdescriptions.ItemId as[Item ID],dbo.ItemDetails.ItemName as[Item Name],dbo.ItemDetails.ItemCompName as[Item Compnay Name],dbo.ItemPriceDetail.MrpPrice as[Mrp], dbo.customerorderdescriptions.price as[Selling Rate], dbo.customerorderdescriptions.quantity as [Quantity], dbo.customerorderdescriptions.totalammount as[Gross Amount], dbo.orderdetails.Discount as[Discount Rate],((dbo.customerorderdescriptions.totalammount*dbo.orderdetails.Discount)/100) as [Discount Amount], dbo.orderdetails.Tax as[Tax], (dbo.customerorderdescriptions.totalammount)- ((dbo.customerorderdescriptions.totalammount)/(1+(dbo.orderdetails.Tax/100))) as [Tax Amount], dbo.orderdetails.totalammount AS [Net Amount (Including Tax)] FROM dbo.CompnayDetails CROSS JOIN dbo.ItemDetails INNER JOIN dbo.customerorderdescriptions ON dbo.ItemDetails.ItemId = dbo.customerorderdescriptions.ItemId INNER JOIN dbo.ItemPriceDetail ON dbo.ItemDetails.ItemId = dbo.ItemPriceDetail.ItemId INNER JOIN dbo.orderdetails ON dbo.customerorderdescriptions.orderid = dbo.orderdetails.orderid INNER JOIN dbo.CustomerDetails ON dbo.orderdetails.custid = dbo.CustomerDetails.custId   where date BETWEEN '" + dateTimePicker1.Value.Date + "' AND '" + dateTimePicker2.Value.Date + "'";
           // string s = comsalesordersearch.SelectedValue.ToString();
            string selectquery = "SELECT dbo.customerorderdescriptions.orderid as [Sales Order ID], dbo.orderdetails.date as [Order Date],dbo.CustomerDetails.custId as[Customer ID], dbo.CustomerDetails.CustName as[Customer Name],dbo.CustomerDetails.CustCompName as[Compnay Name],dbo.customerorderdescriptions.ItemId as[Item ID],dbo.ItemDetails.ItemName as[Item Name],dbo.ItemDetails.ItemCompName as[Item Compnay Name],itd.HSN,cast(dbo.ItemPriceDetail.MrpPrice as numeric(38,2)) as[MRP],cast(dbo.customerorderdescriptions.price as numeric(38,2)) as[Selling Rate],dbo.customerorderdescriptions.quantity as [Quantity Billed],cast(dbo.customerorderdescriptions.price *dbo.customerorderdescriptions.quantity as numeric(38,2))as[Gross Amount],itd.Discount as[Discount Rate (In %)],cast((dbo.customerorderdescriptions.price*itd.Discount)/100 as numeric(38,2)) as [Discount Amount],cast((((dbo.customerorderdescriptions.Price)* (dbo.customerorderdescriptions.Quantity))-(dbo.customerorderdescriptions.Price)* (dbo.customerorderdescriptions.Quantity)*(itd.Discount/100))as numeric(38,2)) as [Taxable Value],( case when DBo.CustomerDetails.CustState != DBO.CompnayDetails.State then '0.0'else CGST end) as CGST,(case when DBo.CustomerDetails.CustState != DBO.CompnayDetails.State then '0.0'else SGST end) as SGST,(case when DBo.CustomerDetails.CustState = DBO.CompnayDetails.State then '0.0' else itd.IGST  end) as IGST, itd.CESS, cast((dbo.customerorderdescriptions.totalammount)-((dbo.customerorderdescriptions.totalammount*dbo.orderdetails.Discount)/100)as numeric(38,2)) AS [Net Amount (Including Tax)],(case when exists  ( select orderid from salesOrderDelivery where Orderid=  orderdetails.orderid)then 'Delivered'else 'Pending'end) as [Delivery Status] FROM dbo.CompnayDetails CROSS JOIN dbo.ItemDetails INNER JOIN dbo.customerorderdescriptions ON dbo.ItemDetails.ItemId = dbo.customerorderdescriptions.ItemId INNER JOIN dbo.ItemPriceDetail ON dbo.ItemDetails.ItemId = dbo.ItemPriceDetail.ItemId Join ItemTaxDetail itd on dbo.ItemPriceDetail.ItemId=itd.ItemId INNER JOIN dbo.orderdetails ON dbo.customerorderdescriptions.orderid = dbo.orderdetails.orderid INNER JOIN dbo.CustomerDetails ON dbo.orderdetails.custid = dbo.CustomerDetails.custId where date BETWEEN '" + dateTimePicker1.Value.Date + "' AND '" + dateTimePicker2.Value.Date + "'";
            DataTable dt = d.getDetailByQuery(selectquery);
            dataGridView1.DataSource = dt;
            double s1 = getTotalAmounts();
            txtGrossAmount.Text = s1.ToString();

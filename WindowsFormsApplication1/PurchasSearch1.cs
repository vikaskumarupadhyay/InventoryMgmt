...
namespace WindowsFormsApplication1
{
    public partial class PurchasSearch1 : Form
    {
        ...
        private void PurchasSearch_Load(object sender, EventArgs e)
        {
            ...
            string selectqurry = "SELECT dbo.CustomerOrderDelivery.Deliveryid as[Invoice ID], dbo.CustomerOrderDelivery.RefNo as[Order Ref. NO], dbo.VendorDetails.venderId as[Vendor ID],dbo.VendorDetails.vName as[Vendor Name], dbo.VendorDetails.vCompName as[Vendor Company Name], dbo.VendorDetails.vAddress as[Vendor Address], dbo.CustomerOrderDelivery.DeliveryDate as[Invoice Date], dbo.VendorOrderDesc.ItemId as[Item ID], dbo.ItemDetails.ItemName as[Item Name], dbo.ItemDetails.ItemCompName as[Item Company Name], cast(dbo.ItemPriceDetail.MrpPrice as numeric(38,2)) as[MRP], cast(dbo.VendorOrderDesc.Price as numeric(38,2)) as[Selling Rate], dbo.VendorOrderDesc.Quantity as[Quantity Billed], cast(dbo.VendorOrderDesc.TotalPrice as numeric(38,2)) AS [Gross Amount], dbo.VendorOrderDetails.Discount as[Discount Rate],cast((dbo.VendorOrderDesc.TotalPrice*dbo.VendorOrderDetails.Discount)/100 as numeric(38,2)) as [Discount Amount], dbo.VendorOrderDetails.Vat as[Tax], cast((dbo.VendorOrderDesc.TotalPrice)- ((dbo.VendorOrderDesc.TotalPrice)/(1+(dbo.VendorOrderDetails.Vat/100)))as numeric(38,2)) as [Tax Amount], cast(( dbo.VendorOrderDesc.TotalPrice)-((dbo.VendorOrderDesc.TotalPrice*dbo.VendorOrderDetails.Discount)/100)as numeric(38,2)) as[Net Amount (Including Tax)],(case when payment.BalanceAmount>0 then 'Delivered' else 'Fully settled' end) as [Delivery Status] FROM   dbo.CustomerOrderDelivery INNER JOIN dbo.VendorOrderDetails ON dbo.CustomerOrderDelivery.Orderid = dbo.VendorOrderDetails.Orderid Inner JOIN AllPaymentDetailes payment on payment.Invoiceid= dbo.CustomerOrderDelivery.Deliveryid INNER JOIN dbo.VendorDetails ON dbo.VendorOrderDetails.venderId = dbo.VendorDetails.venderId INNER JOIN dbo.VendorOrderDesc ON dbo.VendorOrderDetails.Orderid = dbo.VendorOrderDesc.Orderid INNER JOIN dbo.ItemDetails ON dbo.VendorOrderDesc.ItemId = dbo.ItemDetails.ItemId INNER JOIN dbo.ItemPriceDetail ON dbo.ItemDetails.ItemId = dbo.ItemPriceDetail.ItemId ORDER BY Deliveryid ASC";
            string selectqurryForActualColumnName = "select top 1 dbo.CustomerOrderDelivery.Deliveryid, dbo.CustomerOrderDelivery.RefNo, dbo.VendorDetails.venderId,dbo.VendorDetails.vName, dbo.VendorDetails.vCompName, dbo.VendorDetails.vAddress, dbo.CustomerOrderDelivery.DeliveryDate, dbo.VendorOrderDesc.ItemId, dbo.ItemDetails.ItemName, dbo.ItemDetails.ItemCompName, dbo.ItemPriceDetail.MrpPrice, dbo.VendorOrderDesc.Price, dbo.VendorOrderDesc.Quantity, dbo.VendorOrderDesc.TotalPrice, dbo.VendorOrderDetails.Discount,cast((dbo.VendorOrderDesc.TotalPrice*dbo.VendorOrderDetails.Discount)/100 as numeric(38,2)), dbo.VendorOrderDetails.Vat, cast((dbo.VendorOrderDesc.TotalPrice)- ((dbo.VendorOrderDesc.TotalPrice)/(1+(dbo.VendorOrderDetails.Vat/100)))as numeric(38,2)), cast(( dbo.VendorOrderDesc.TotalPrice)-((dbo.VendorOrderDesc.TotalPrice*dbo.VendorOrderDetails.Discount)/100)as numeric(38,2))as[Net Amount (Including Tax)],(case when payment.BalanceAmount>0 then 'Delivered' else 'Fully settled' end) as [Delivery Status] FROM   dbo.CustomerOrderDelivery INNER JOIN dbo.VendorOrderDetails ON dbo.CustomerOrderDelivery.Orderid = dbo.VendorOrderDetails.Orderid Inner JOIN AllPaymentDetailes payment on payment.Invoiceid= dbo.CustomerOrderDelivery.Deliveryid INNER JOIN dbo.VendorDetails ON dbo.VendorOrderDetails.venderId = dbo.VendorDetails.venderId INNER JOIN dbo.VendorOrderDesc ON dbo.VendorOrderDetails.Orderid = dbo.VendorOrderDesc.Orderid INNER JOIN dbo.ItemDetails ON dbo.VendorOrderDesc.ItemId = dbo.ItemDetails.ItemId INNER JOIN dbo.ItemPriceDetail ON dbo.ItemDetails.ItemId = dbo.ItemPriceDetail.ItemId";
            DataTable dt = dbMainClass.getDetailByQuery(selectqurry);

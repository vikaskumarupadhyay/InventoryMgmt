...
namespace WindowsFormsApplication1
{
    public partial class Form9 : Form
    {
        ...
        {
            string selectqurry = "SELECT cod.Deliveryid as[Purchase Invoice ID], payment.PaymentDate as[payment date],vd.venderId as[Vendor ID],vd.vName as[Vendor Name],vd.vCompName as[Company Name], vd.vAddress as[Address],vode.Quantity as [Quantity Billed],cast((vode.Price)* (vode.Quantity) as numeric(38,2)) AS [Gross Amount],vod.DisAmount as [Discount Amount],cast((((vode.Price)* (vode.Quantity))-(vode.Price)* (vode.Quantity)*(itd.Discount/100))as numeric(38,2)) as [Taxable Value],vod.TextTaxAmmount as[GST_CESS IN AMOUNT], cast((vode.TotalPrice) - ((vode.TotalPrice *vod.Discount) / 100) as numeric(38, 2)) AS[Net Amount (Including Tax)] ,p.InvoiceAmount as [Invoice Amount] ,p.[Paid Amount] ,p.Balance as[Balance Amount],(case when p.Balance > 0 then 'Delivered' else 'Fully settled' end) as [Delivery Status] FROM CustomerOrderDelivery cod INNER JOIN VendorOrderDetails vod ON cod.Orderid = vod.orderid INNER JOIN VendorDetails vd ON vod.venderId = vd.venderId INNER JOIN VendorOrderDesc vode ON vod.orderid = vode.Orderid  INNER JOIN ItemDetails id ON vode.ItemId = id.ItemId  INNER JOIN ItemPriceDetail ipd ON id.ItemId = ipd.ItemId join ItemTaxDetail itd on itd.ItemId = ipd.ItemId left join  (select Invoiceid, MAX(InvoiceAmount) as InvoiceAmount, (MAX(InvoiceAmount) - sum(TotalAmount)) as Balance, sum(TotalAmount) as [Paid Amount] from AllPaymentDetailes  join CustomerOrderDelivery on AllPaymentDetailes.Invoiceid = CustomerOrderDelivery.Deliveryid group by Invoiceid) p on cod.Deliveryid = p.Invoiceid join AllPaymentDetailes payment on cod.Deliveryid=payment.Invoiceid";
            string selectqurryForActualColumnName = "SELECT top 1 cod.Deliveryid,payment.PaymentDate,vd.venderId,vd.vName,vd.vCompName, vd.vAddress,vode.Quantity ,cast((vode.Price)* (vode.Quantity) as numeric(38,2)),vod.DisAmount,cast((((vode.Price)* (vode.Quantity))-(vode.Price)* (vode.Quantity)*(itd.Discount/100))as numeric(38,2)),vod.TextTaxAmmount, cast((vode.TotalPrice) - ((vode.TotalPrice *vod.Discount) / 100) as numeric(38, 2)),p.InvoiceAmount ,p.[Paid Amount] ,p.Balance,(case when p.Balance > 0 then 'Delivered' else 'Fully settled' end)FROM CustomerOrderDelivery cod INNER JOIN VendorOrderDetails vod ON cod.Orderid = vod.orderid INNER JOIN VendorDetails vd ON vod.venderId = vd.venderId INNER JOIN VendorOrderDesc vode ON vod.orderid = vode.Orderid  INNER JOIN ItemDetails id ON vode.ItemId = id.ItemId  INNER JOIN ItemPriceDetail ipd ON id.ItemId = ipd.ItemId join ItemTaxDetail itd on itd.ItemId = ipd.ItemId left join  (select Invoiceid, MAX(InvoiceAmount) as InvoiceAmount, (MAX(InvoiceAmount) - sum(TotalAmount)) as Balance, sum(TotalAmount) as [Paid Amount] from AllPaymentDetailes  join CustomerOrderDelivery on AllPaymentDetailes.Invoiceid = CustomerOrderDelivery.Deliveryid group by Invoiceid) p on cod.Deliveryid = p.Invoiceid join AllPaymentDetailes payment on cod.Deliveryid=payment.Invoiceid";
        ...
        private void setDetails(DataGridViewCellCollection cellCollection)
        {
            try
            {
               ...
               // txttotalAmount.Text = cellCollection[14].Value.ToString();
                ...
                amount = Convert.ToDouble(cellCollection[14].Value.ToString());
            }
            catch (Exception ex)
            {
            }

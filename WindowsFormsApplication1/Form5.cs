...
namespace WindowsFormsApplication1
{
    public partial class Form5 : Form
    {
        ...
        public void pageloadsave()
        {
            ...
            string selectqurryForActualColumnName = "SELECT top 1 dbo.salesOrderDelivery.Delivaryid ,dbo.salesOrderDelivery.DeliveryDate, dbo.CustomerDetails.custId, dbo.CustomerDetails.CustName, dbo.CustomerDetails.CustCompName,dbo.CustomerDetails.CustAddress,dbo.customerorderdescriptions.quantity ,cast(dbo.customerorderdescriptions.price *dbo.customerorderdescriptions.quantity as numeric(38,2)),itd.Discount,cast((dbo.customerorderdescriptions.price*itd.Discount)/100 as numeric(38,2)),cast((((dbo.customerorderdescriptions.Price)* (dbo.customerorderdescriptions.Quantity))-(dbo.customerorderdescriptions.Price)* (dbo.customerorderdescriptions.Quantity)*(itd.Discount/100))as numeric(38,2)),(case when DBo.CustomerDetails.CustState != (select top 1 State from DBO.CompnayDetails) then '0.0'else CGST end),(case when DBo.CustomerDetails.CustState != (select top 1 State from DBO.CompnayDetails) then '0.0'else SGST end),(case when DBo.CustomerDetails.CustState = (select top 1 State from DBO.CompnayDetails) then '0.0' else itd.IGST  end), itd.CESS, cast((dbo.customerorderdescriptions.totalammount) - ((dbo.customerorderdescriptions.totalammount * dbo.orderdetails.Discount) / 100) as numeric(38, 2)) ,p.InvoiceAmount ,p.[Paid Amount] ,p.Balance,(case when p.Balance > 0 then 'Delivered' else 'Fully settled' end) FROM dbo.salesOrderDelivery INNER JOIN dbo.orderdetails ON dbo.salesOrderDelivery.Orderid = dbo.orderdetails.orderid INNER JOIN dbo.CustomerDetails ON dbo.orderdetails.custid = dbo.CustomerDetails.custId   INNER JOIN dbo.customerorderdescriptions ON dbo.orderdetails.orderid = dbo.customerorderdescriptions.Orderid  INNER JOIN dbo.ItemDetails ON dbo.customerorderdescriptions.ItemId = dbo.ItemDetails.ItemId  INNER JOIN dbo.ItemPriceDetail ON dbo.ItemDetails.ItemId = dbo.ItemPriceDetail.ItemId Join ItemTaxDetail itd on dbo.ItemPriceDetail.ItemId=itd.ItemId  left join  (select Invoiceid, MAX(InvoiceAmount) as InvoiceAmount, (MAX(InvoiceAmount) - sum(TotalAmount)) as Balance, sum(TotalAmount) as [Paid Amount] from SalesPaymentDetailes  join salesOrderDelivery on SalesPaymentDetailes.Invoiceid = salesOrderDelivery.Delivaryid  group by Invoiceid) p on salesOrderDelivery.Delivaryid = p.Invoiceid";
        ...
        private void setdetails(DataGridViewCellCollection collection)
        {
            try
            {
                ...
                amount = Convert.ToDouble(collection[18].Value.ToString());
            }
            catch (Exception ex)
            {

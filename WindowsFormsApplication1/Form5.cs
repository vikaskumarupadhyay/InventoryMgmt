...
namespace WindowsFormsApplication1
{
    public partial class Form5 : Form
    {
        ...
        private void dataGridView2_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                setdetails(cell);
                string selectquery = " SELECT pay.ReceiptDate as[Receipt Date],cod.quantity as [Quantity Billed],cast(cod.price *cod.quantity as numeric(38,2))as[Gross Amount],itd.Discount as[Discount Rate (In %)],cast((cod.price*itd.Discount)/100 as numeric(38,2)) as [Discount Amount],cast((((cod.Price)* (cod.Quantity))-(cod.Price)* (cod.Quantity)*(itd.Discount/100))as numeric(38,2)) as [Taxable Value],(case when cd.CustState != (select top 1 State from DBO.CompnayDetails) then '0.0'else CGST end) as CGST,(case when cd.CustState != (select top 1 State from DBO.CompnayDetails) then '0.0'else SGST end) as SGST,(case when cd.CustState = (select top 1 State from DBO.CompnayDetails) then '0.0' else itd.IGST  end) as IGST, itd.CESS, cast((cod.totalammount) - ((cod.totalammount * od.Discount) / 100) as numeric(38, 2)) AS[Net Amount (Including Tax)] ,p.InvoiceAmount as [Invoice Amount] ,p.[Paid Amount] ,p.Balance as [Balance Amount] FROM salesOrderDelivery sod INNER JOIN orderdetails od ON sod.Orderid = od.orderid INNER JOIN CustomerDetails cd ON od.custid = cd.custId   INNER JOIN customerorderdescriptions cod ON od.orderid = cod.Orderid  INNER JOIN ItemDetails id ON cod.ItemId = id.ItemId  INNER JOIN dbo.ItemPriceDetail ON id.ItemId = dbo.ItemPriceDetail.ItemId Join ItemTaxDetail itd on dbo.ItemPriceDetail.ItemId=itd.ItemId  left join  (select Invoiceid, MAX(InvoiceAmount) as InvoiceAmount, (MAX(InvoiceAmount) - sum(TotalAmount)) as Balance, sum(TotalAmount) as [Paid Amount] from SalesPaymentDetailes  join salesOrderDelivery on SalesPaymentDetailes.Invoiceid = salesOrderDelivery.Delivaryid  group by Invoiceid) p on sod.Delivaryid = p.Invoiceid join SalesPaymentDetailes pay on sod.Delivaryid=pay.Invoiceid where sod.Delivaryid='" + txtRefNo.Text + "' ";
                DataTable dt = d.getDetailByQuery(selectquery);
                panel2.Visible = false;
        ...
        private void setdetails(DataGridViewCellCollection collection)
        {
            try
            {
                ...
                amount = Convert.ToDouble(collection[14].Value.ToString());
            }
            catch (Exception ex)
            {

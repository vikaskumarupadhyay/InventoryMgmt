...
namespace WindowsFormsApplication1
{
    public partial class Form5 : Form
    {
        ...
        private void textBox8_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                s = "cast((orderdetails.WithautTaxamount-orderdetails.Discountamount)as numeric(38, 2))";
            }
            else if (s == "Column3")
            {
                s = "Case when cd.CustState != (select top 1 [State] from CompnayDetails )then '0'else (select  sum ((((( COD.price*COD.quantity))-( ( ( COD.price*COD.quantity) * IT.Discount) / 100 ))* IT.CGST )/100)from ItemTaxDetail IT JOIN customerorderdescriptions COD  ON  IT.ItemId=COD.ItemId WHERE COD.orderid=orderdetails.orderid) end";
            }
            else if (s == "Column4")
            {
                s = "Case when cd.CustState != (select top 1 [State] from CompnayDetails )then '0'else(select  sum ((((( COD.price*COD.quantity)) -  ( ( ( COD.price*COD.quantity) * IT.Discount) / 100 ) ) * IT.SGST )/100) from ItemTaxDetail IT JOIN customerorderdescriptions COD  ON  IT.ItemId=COD.ItemId WHERE COD.orderid=orderdetails.orderid)end";
            }
            else if (s == "Column5")
            {
                s = "Case when cd.CustState = (select top 1 [State] from CompnayDetails )then '0'else(select sum ((((( COD.price*COD.quantity)) -  ( ( ( COD.price*COD.quantity) * IT.Discount) / 100 ) ) * IT.IGST )/100) from ItemTaxDetail IT JOIN customerorderdescriptions COD  ON  IT.ItemId=COD.ItemId WHERE COD.orderid=orderdetails.orderid)end";
            }
            else if (s == "Column6")
            {
                s = "(select  sum ((((( COD.price*COD.quantity)) -  ( ( ( COD.price*COD.quantity) * IT.Discount) / 100 ) ) * IT.CESS )/100) from ItemTaxDetail IT JOIN customerorderdescriptions COD  ON  IT.ItemId=COD.ItemId WHERE COD.orderid=orderdetails.orderid)";
            }
